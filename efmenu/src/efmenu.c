...
static void show_menu(uint8_t n_page, uint8_t full_update)
{
    ...
    if (full_update)
    {
        erase_text_areas(COLOR_BLACK << 4 | COLOR_BLACK);
        fill_directory();
    }
        ...
        if (menu->n_max_entries && menu->n_page == n_page) /* hidden otherwise */
        {
            ...
            while (entry->key)
            {
                ...
                if (entry == get_current_menu_entry())
                    color = COLOR_BLACK << 4 | COLOR_GREEN;
                else if (menu_entry_is_valid(entry))
                    color = COLOR_WHITE << 4 | COLOR_BLACK;
                else
                    color = COLOR_GRAY2 << 4 | COLOR_BLACK;
...
static void show_version(void)
{
    ...
    erase_text_areas(COLOR_WHITE << 4 | COLOR_BLACK);
